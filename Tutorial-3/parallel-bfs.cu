 ...
 * to the serial-bfs.c implementation to utilize gpu's computing capability
...
/*
 * Helper Function to find the BFS from source vertex on graph
 * which is called in a loop to find the distance in an iterative manner
 */
__global__ static void HBFS(struct Graph *graph, int level) {
  int vertex = (blockIdx.x * blockDim.x) + threadIdx.x;

  // bounds checking
  if (vertex >= graph->size || graph->dist[vertex] != level)
    return;
	
	int start = graph->offset[vertex];
	int end = (vertex == graph->size - 1) ? graph->edges : graph->offset[vertex + 1];
	
	for(int i = start; i < end; i++) {
		struct Data data = graph->csr[i];

		int old = atomicMin(&graph->dist[data.dest], level + 1);
...
int main(int argc, char **argv) {
  ...
  printf(
      "Parallely Computing Single Source Shortest Path on device (BFS) from "
      "vertex [%d]\n",
      src);
  ...
  int h_changed = 1;                  // flag for early exit
	int level = 0; // level indicator for bfs search
	
  // calculate the distance by BFS algorithm
	while(h_changed) {
		h_changed = 0;
		// intialize device changed from host changed
		cudaMemcpyToSymbol(d_changed, &h_changed, sizeof(int), 0, cudaMemcpyHostToDevice);
	
		// run the algorithm
		HBFS<<<num_blocks, num_threads>>>(d_graph, level);
		
		// copy back device changed to host changed
